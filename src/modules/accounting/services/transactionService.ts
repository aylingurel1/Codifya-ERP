...
export class TransactionService {
  ...
  async getTransactions(
    ...
    // Tip dönüşümü - Prisma verilerini Transaction tipine uygun hale getir
  ...
  async getFinancialSummary(): Promise<FinancialSummary> {
    ...
    for (const item of incomeByCategoryArr) {
      incomeByCategory[item.category as TransactionCategory] =
    ...
    for (const item of expenseByCategoryArr) {
      expenseByCategory[item.category as TransactionCategory] =
        Number(item._sum.amount) || 0;
    }
    ...
    return {
      ...
      netProfit:
        ...
        (Number(totalExpense._sum.amount) || 0),
      incomeByCategory,
      expenseByCategory,
